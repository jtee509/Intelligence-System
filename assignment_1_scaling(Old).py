...
from sklearn.linear_model import LinearRegression #importing LinearRegression from linear_model
...
# min_bmi = dataset['bmi'].min()

# # # Calculate the range
# range_bmi = max_bmi - min_bmi

# dataset = dataset.with_columns((pl.col('bmi') - mean_bmi) / (range_bmi))

bmi = dataset['bmi'] #X
charges = dataset['charges'] #Y
...
no_example = len(bmi) #number of training examples
...
while epoch_counter <= epochs: 
    ...
    if epoch_counter == epochs:
        endtime = datetime.now()
        break
    theta0, theta1 = gradient_descent(theta0, theta1, no_example, alpha1)
    epoch_counter += 1  
...
with open('output_0.01_withoutScaled.csv', 'w', newline='') as f:
    writer = csv.writer(f)
